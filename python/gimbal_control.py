...
if __name__ == "__main__":
    ...
    rate=rospy.Rate(30)
    gimbal_pitch_ = -18 #-15 #-60
    gimbal_yaw_ = 0 #0.0
    gimbal_roll_ = 0.0
    srvheader=std_msgs.msg.Header()
    srvheader.stamp=rospy.Time.now()
    srvheader.frame_id="map"
    cam_pose_pub = rospy.Publisher(vehicle_type+'_'+vehicle_id+'/cam_pose', PoseStamped, queue_size=1)
    cam_pose = PoseStamped()
    gazeboLinkstate = rospy.ServiceProxy('gazebo/get_link_state', GetLinkState)

    #mountConfig(header=srvheader,mode=2,stabilize_roll=0,stabilize_yaw=0.0,stabilize_pitch=0)
