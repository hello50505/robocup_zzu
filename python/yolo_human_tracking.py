...
from publishBox.msg import BoundingBox
import math
...
'''
def darknet_callback(target):
    global find_cnt, twist, cmd, target_height_mask, target_height,theta, get_time
    print(target.goal_Id)
    if target.goal_Id in idlist:
        z = height / math.sin(theta)
        u = (target.xmax+target.xmin)/2
        v = (target.ymax+target.ymin)/2
        u_ = u-u_center
...
if __name__ == "__main__":
    ...
    cmd = String()
    theta = 0
    u_center=320.5#640/2 
    v_center=240.5#360/2
    fx = 554.254691191187#205.46963709898583
    fy = 554.254691191187#205.46963709898583
    Kp_xy = 0.20#0.5
    Kp_z = 1
    ...
    #rospy.Subscriber("/typhoon_h480_"+vehicle_id+"/darknet_ros/bounding_boxes", BoundingBoxes, darknet_callback,queue_size=1)
    rospy.Subscriber("/typhoon_h480_"+vehicle_id+"/bounding_box", BoundingBox, darknet_callback,queue_size=1)
    rospy.Subscriber(vehicle_type+'_'+vehicle_id+"/mavros/local_position/pose", PoseStamped, local_pose_callback,queue_size=1)
    rospy.Subscriber('/'+vehicle_type+'_'+vehicle_id+'/cam_pose', PoseStamped, cam_pose_callback,queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if find_cnt - find_cnt_last == 0:
            if not get_time:
                ...
                get_time = True
            if rospy.get_time() - not_find_time > 1.0:
                twist.linear.x = 0.0
                twist.linear.y = 0.0
                twist.linear.z = 0.0
                cmd = 'HOVER'
